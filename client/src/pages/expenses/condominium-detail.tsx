...
// Specific subcategories for condominium
const CONDOMINIUM_SUBCATEGORIES = [
  'Taxa Condominial', 'Fundo de Reserva', 'Obras Extraordinárias', 
  'Multas Condominiais', 'Seguros', 'Administração Predial', 'Outros Condomínio'
];
...
interface CondominiumDetailRow {
  propertyName: string;
  monthlyData: { [monthKey: string]: { amount: number; transactions: Transaction[] } };
  total: number;
}

interface CondominiumDetailData {
  rows: CondominiumDetailRow[];
  monthHeaders: string[];
...
export default function CondominiumDetailPage() {
  ...
  useEffect(() => {
    if (!filtersInitialized && allProperties.length > 0) {
      ...
      // Subcategories start empty (only condominium category)
  ...
  // Generate condominium detail data with intelligent mapping
  const generateCondominiumDetailData = (): CondominiumDetailData => {
    const detailData: Record<string, Record<string, { amount: number; transactions: Transaction[] }>> = {};
    
    // Filter transactions by condominium category with smart mapping
    const condominiumTransactions = allTransactions.filter((transaction: any) => {
      let effectiveCategory = transaction.category;
      
      // Apply same smart mapping as main expenses page
      // If transaction is in utilities but contains "condomin" in description, treat as condominium
      ...
      return effectiveCategory === 'condominium';
    });

    condominiumTransactions.forEach((transaction: any) => {
    ...
    const rows: CondominiumDetailRow[] = [];
  ...
  const condominiumDetailData = generateCondominiumDetailData();
  ...
  const sortedRows = [...condominiumDetailData.rows].sort((a, b) => {
    ...
    if (key === 'propertyName') {
      ...
      aValue = a.total / condominiumDetailData.monthHeaders.length;
      bValue = b.total / condominiumDetailData.monthHeaders.length;
    } else {
  ...
  const handleCellClick = (propertyName: string, monthKey: string) => {
    ...
    const monthData = condominiumDetailData.rows.find(r => r.propertyName === propertyName)?.monthlyData[monthKey];
  ...
  const exportToExcel = () => {
    ...
    const headers = ['Propriedade', ...condominiumDetailData.monthHeaders];
    
    if (condominiumDetailData.monthHeaders.length > 1) {
      headers.push(`Média Mensal (÷${condominiumDetailData.monthHeaders.length})`);
    }
    ...
    sortedRows.forEach(row => {
      const rowData = [
        row.propertyName,
        ...condominiumDetailData.monthHeaders.map(month => {
      ...
      if (condominiumDetailData.monthHeaders.length > 1) {
        rowData.push(-row.total / condominiumDetailData.monthHeaders.length);
      }
    ...
    const totalsRow = [
      'TOTAL',
      ...condominiumDetailData.monthHeaders.map(month => -condominiumDetailData.columnTotals[month])
    ];
    
    if (condominiumDetailData.monthHeaders.length > 1) {
      totalsRow.push(-condominiumDetailData.grandTotal / condominiumDetailData.monthHeaders.length);
    }
    
    totalsRow.push(-condominiumDetailData.grandTotal);
    data.push(totalsRow);
    ...
    condominiumDetailData.monthHeaders.forEach(() => colWidths.push({ wch: 12 }));
    ...
    const fileName = `Condominio_detalhes_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  const exportToPDF = () => {
    ...
    doc.setFontSize(16);
    doc.text('Condomínio - Detalhes por Propriedade', 20, 20);
    ...
    const data = sortedRows.map(row => {
      ...
      if (condominiumDetailData.monthHeaders.length > 1) {
        rowData.push(formatNumber(row.total / condominiumDetailData.monthHeaders.length));
      }
    ...
    const totalsRow = [
      'TOTAL',
      ...condominiumDetailData.monthHeaders.map(month => formatNumber(condominiumDetailData.columnTotals[month]))
    ];
    
    if (condominiumDetailData.monthHeaders.length > 1) {
      totalsRow.push(formatNumber(condominiumDetailData.grandTotal / condominiumDetailData.monthHeaders.length));
    }
    
    totalsRow.push(formatNumber(condominiumDetailData.grandTotal));
    data.push(totalsRow);
    ...
    const fileName = `Condominio_detalhes_${new Date().toISOString().split('T')[0]}.pdf`;
    doc.save(fileName);
          ...
          <Collapsible open={isFiltersOpen} onOpenChange={setIsFiltersOpen}>
            ...
            <CollapsibleContent className="mt-4 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-gray-50 rounded-lg">
                ...
                {/* Subcategory Filter */}
                <div className="space-y-2">
                  <label className="text-sm font-medium">Tipos de Condomínio</label>
                  <div className="max-h-40 overflow-y-auto border rounded p-2 bg-white">
                    {CONDOMINIUM_SUBCATEGORIES.map(subcategory => (
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="text-sm text-muted-foreground">
                  {condominiumDetailData.rows.length} propriedades | Total: {formatNumber(condominiumDetailData.grandTotal)}
                </div>
              </div>
            </CollapsibleContent>
          </Collapsible>
            ...
            <table className="w-full border-collapse border border-gray-200">
              <thead>
                <tr className="bg-gray-50">
                  ...
                  {condominiumDetailData.monthHeaders.length > 1 && (
                    <th className="border border-gray-200 p-3 text-center font-medium bg-blue-50">
                      <button 
                        onClick={() => handleSort('average')}
                        className="flex items-center gap-2 hover:text-blue-600 mx-auto"
                      >
                        Média Mensal (÷{condominiumDetailData.monthHeaders.length})
                    ...
                    <td className="border border-gray-200 p-3 font-medium">
                      <button
                        ...
                        onClick={() => {
                          ...
                          setDetailModalTitle(`Condomínio - ${row.propertyName}`);
                    ...
                    {condominiumDetailData.monthHeaders.length > 1 && (
                      <td className="border border-gray-200 p-3 text-center font-semibold bg-blue-50">
                        <span className="text-red-600">
                          {formatNumber(row.total / condominiumDetailData.monthHeaders.length)}
                        </span>
                      </td>
                    )}
                  ...
                  {condominiumDetailData.monthHeaders.map(month => (
                    <td key={month} className="border border-gray-200 p-3 text-center">
                      <span className="text-red-600 font-bold">
                        {formatNumber(condominiumDetailData.columnTotals[month])}
                      </span>
                    </td>
                  ))}
                  {condominiumDetailData.monthHeaders.length > 1 && (
                    <td className="border border-gray-200 p-3 text-center bg-blue-50">
                      <span className="text-red-600 font-bold">
                        {formatNumber(condominiumDetailData.grandTotal / condominiumDetailData.monthHeaders.length)}
                      </span>
                    </td>
                  )}
                  <td className="border border-gray-200 p-3 text-center">
                    <span className="text-red-600 font-bold">
                      {formatNumber(condominiumDetailData.grandTotal)}
                    </span>
                  </td>
                </tr>
              </tfoot>
