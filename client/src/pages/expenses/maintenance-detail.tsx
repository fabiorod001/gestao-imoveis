...
const MAINTENANCE_SUBCATEGORIES = [
  'Conserto/Manutenção', 'Reformas', 'Pintura', 'Dedetização', 
  'Jardinagem', 'Limpeza', 'Reparos', 'Outros Manutenção'
];
...
export default function MaintenanceDetailPage() {
  ...
  useEffect(() => {
    if (!filtersInitialized && allProperties.length > 0) {
      ...
      // Subcategories start empty (only maintenance category)
  ...
  const generateMaintenanceDetailData = (): MaintenanceDetailData => {
    ...
    maintenanceTransactions.forEach((transaction: any) => {
    ...
    const rows: MaintenanceDetailRow[] = [];
  ...
  const maintenanceDetailData = generateMaintenanceDetailData();
  ...
  const sortedRows = [...maintenanceDetailData.rows].sort((a, b) => {
    ...
    if (key === 'propertyName') {
      ...
      aValue = a.total / maintenanceDetailData.monthHeaders.length;
      bValue = b.total / maintenanceDetailData.monthHeaders.length;
    } else {
  ...
  const exportToExcel = () => {
    ...
    const headers = ['Propriedade', ...maintenanceDetailData.monthHeaders];
    
    if (maintenanceDetailData.monthHeaders.length > 1) {
      headers.push(`Média Mensal (÷${maintenanceDetailData.monthHeaders.length})`);
    }
    ...
    sortedRows.forEach(row => {
      const rowData = [
        row.propertyName,
        ...maintenanceDetailData.monthHeaders.map(month => {
      ...
      if (maintenanceDetailData.monthHeaders.length > 1) {
        rowData.push(-row.total / maintenanceDetailData.monthHeaders.length);
      }
    ...
    const totalsRow = ['TOTAL', ...maintenanceDetailData.monthHeaders.map(month => -maintenanceDetailData.columnTotals[month])];
    
    if (maintenanceDetailData.monthHeaders.length > 1) {
      totalsRow.push(-maintenanceDetailData.grandTotal / maintenanceDetailData.monthHeaders.length);
    }
    
    totalsRow.push(-maintenanceDetailData.grandTotal);
    data.push(totalsRow);
    ...
    maintenanceDetailData.monthHeaders.forEach(() => colWidths.push({ wch: 12 }));
    ...
    const fileName = `Manutencao_detalhes_${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  const exportToPDF = () => {
    ...
    doc.setFontSize(16);
    doc.text('Manutenção - Detalhes por Propriedade', 20, 20);
    ...
    const data = sortedRows.map(row => {
      ...
      if (maintenanceDetailData.monthHeaders.length > 1) {
        rowData.push(formatNumber(row.total / maintenanceDetailData.monthHeaders.length));
      }
    ...
    const totalsRow = ['TOTAL', ...maintenanceDetailData.monthHeaders.map(month => formatNumber(maintenanceDetailData.columnTotals[month]))];
    
    if (maintenanceDetailData.monthHeaders.length > 1) {
      totalsRow.push(formatNumber(maintenanceDetailData.grandTotal / maintenanceDetailData.monthHeaders.length));
    }
    
    totalsRow.push(formatNumber(maintenanceDetailData.grandTotal));
    data.push(totalsRow);
    ...
    const fileName = `Manutencao_detalhes_${new Date().toISOString().split('T')[0]}.pdf`;
    doc.save(fileName);
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Detalhes: Manutenção</h1>
          ...
          <Collapsible open={isFiltersOpen} onOpenChange={setIsFiltersOpen}>
            ...
            <CollapsibleContent className="mt-4 space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 p-4 bg-gray-50 rounded-lg">
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium">Tipos de Manutenção</label>
                  <div className="max-h-40 overflow-y-auto border rounded p-2 bg-white">
              ...
              <div className="flex justify-between items-center">
                ...
                <div className="text-sm text-muted-foreground">
                  {maintenanceDetailData.rows.length} propriedades | Total: {formatNumber(maintenanceDetailData.grandTotal)}
                </div>
              </div>
            </CollapsibleContent>
          </Collapsible>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Manutenção - Por Propriedade</CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <table className="w-full border-collapse border border-gray-200">
              <thead>
                <tr className="bg-gray-50">
                  ...
                  {maintenanceDetailData.monthHeaders.length > 1 && (
                    <th className="border border-gray-200 p-3 text-center font-medium bg-blue-50">
                      <button onClick={() => handleSort('average')} className="flex items-center gap-2 hover:text-blue-600 mx-auto">
                        Média Mensal (÷{maintenanceDetailData.monthHeaders.length})
                    ...
                    <td className="border border-gray-200 p-3 font-medium">
                      <button
                        ...
                        onClick={() => {
                          ...
                          setDetailModalTitle(`Manutenção - ${row.propertyName}`);
                    ...
                    {maintenanceDetailData.monthHeaders.length > 1 && (
                      <td className="border border-gray-200 p-3 text-center font-semibold bg-blue-50">
                        <span className="text-red-600">
                          {formatNumber(row.total / maintenanceDetailData.monthHeaders.length)}
                        </span>
                      </td>
                    )}
                  ...
                  {maintenanceDetailData.monthHeaders.map(month => (
                    <td key={month} className="border border-gray-200 p-3 text-center">
                      <span className="text-red-600 font-bold">
                        {formatNumber(maintenanceDetailData.columnTotals[month])}
                      </span>
                    </td>
                  ))}
                  {maintenanceDetailData.monthHeaders.length > 1 && (
                    <td className="border border-gray-200 p-3 text-center bg-blue-50">
                      <span className="text-red-600 font-bold">
                        {formatNumber(maintenanceDetailData.grandTotal / maintenanceDetailData.monthHeaders.length)}
                      </span>
                    </td>
                  )}
                  <td className="border border-gray-200 p-3 text-center">
                    <span className="text-red-600 font-bold">{formatNumber(maintenanceDetailData.grandTotal)}</span>
                  </td>
                </tr>
              </tfoot>
            </table>
